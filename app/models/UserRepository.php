...
class UserRepository
{
    ...
    {
        $user = new User;
        $options = array('role_ids' => explode(',', strlen(array_get($input, 'role_ids')) > 0 ? array_get($input, 'role_ids') : '' ));
        $user->email    = array_get($input, 'email');
        $user->password = array_get($input, 'password');
        $user->username = array_get($input, 'username');
        $user->station_id = array_get($input, 'station');
        ...
        // Save if valid. Password field will be hashed before save
         ...
         {
            $user->roles()->attach($options['role_ids']);
         }
        return $user;
    }
     ...
     * @return  boolean Success
     */
    public function save(User $instance, array $options = array())
    {
